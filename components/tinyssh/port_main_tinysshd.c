...
static __thread struct buf b1;
static __thread struct buf b2;
...
int main_tinysshd(int argc, char **argv) {
    ...
    // Note: b1 and b2 will be initialized after global_init() allocates their workspaces
    ...
    keydir = *argv; // On ESP-IDF, keydir is the last argument or port-defined
    ...
    // Dynamically initialize b1 and b2 to point to our thread-local workspaces
    b1.buf = global_bspace1;
    b1.len = 0;
    b1.alloc = GLOBAL_BSIZE;

    b2.buf = global_bspace2;
    b2.len = 0;
    b2.alloc = GLOBAL_BSIZE;
